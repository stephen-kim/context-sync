...
- Runs MCP schema snapshot guard (`tools-schema.snapshot.test.ts`) and reports status in the comment.
- Uploads eval artifacts even when snapshot guard fails, then fails the job at the end.
