...
- CI/local lint runs `scripts/check-route-zod-boundary.mjs` to prevent `req.body/query/params` from being passed to service inputs without a route-level Zod parse.
- Service-layer validation remains in place as a second safety boundary for domain invariants.
